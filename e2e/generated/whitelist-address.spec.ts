// Auto-generated from: features/whitelist-address.feature
// Generated at: 2026-03-10T01:47:20.479Z
// DO NOT EDIT — regenerate with /generate-e2e whitelist-address

import { test, expect } from '@playwright/test';
import e2eConfig from '../../config/e2e.config.json' with { type: 'json' };
import { generateTOTP } from '../helpers/totp-helper.js';

test.describe('Whitelist Address', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.goto(e2eConfig.pages['whitelist-address'] || '/whitelist-address');
    await page.waitForLoadState('networkidle');
  });

  test('Whitelist page loads', async ({ page }) => {
    const ctx = { variables: new Map<string, any>(), lastApiResponse: null as any };
    await expect(page.getByText('Whitelist Address').first()).toBeVisible();
    await expect(page.getByText('Search by wallet name').first()).toBeVisible();
  });

  test('Create wallet button is visible', async ({ page }) => {
    const ctx = { variables: new Map<string, any>(), lastApiResponse: null as any };
    await expect(page.getByText('Create wallet').first()).toBeVisible();
  });
